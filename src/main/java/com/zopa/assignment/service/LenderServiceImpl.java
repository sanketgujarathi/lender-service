...
public class LenderServiceImpl implements LenderService {
    ...
    private Quote calculateQuote(BigDecimal loanAmount, BigDecimal interestRate) {
        ...
        return new Quote(loanAmount, interestRate, monthlyRepayment, monthlyRepayment.multiply(valueOf(DURATION))
    ...
    private boolean isValidAmount(BigDecimal requestedAmount) {
        return requestedAmount.compareTo(valueOf(MIN_VAL)) >= 0 &&
                requestedAmount.compareTo(valueOf(MAX_VAL)) <= 0 &&
                requestedAmount.remainder(valueOf(MULTIPLICITY_FACTOR))
                        .compareTo(ZERO) == 0;
    }
